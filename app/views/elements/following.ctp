<!-- contents_Start -->
<h3>フォローしているユーザ<?php
echo "(" . h($following_user[0]['Count']) . ")"?></h3>
<div class="sideMid">
	<div class="sideBtm">
		<table cellpadding="0" cellspacing="0" class='wordBreak'>
<?php
foreach ($following_user as $key => $val) {
...
if ($following_user[0]['Count'] > 5) {
	...
	if (isset($profile)) {
		echo $customJs->link($html->image('common/bt_more_s.gif', array(
			...
			'action' => 'following_user/' . $profile['User']['USR_ID']
		), array(
			'escape' => false,
			'update' => null,
		...
		echo $customJs->link($html->image('common/bt_more_s.gif', array(
			...
			'controller' => 'timelines',
			'action' => 'following_user/' . $user['User']['USR_ID']
		), array(
			'escape' => false,
			'update' => null,
			'complete' => "popupclass.popup_view(XMLHttpRequest);popupclass.popup_open()",
